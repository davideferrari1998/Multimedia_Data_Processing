...
int main() 
{
...

	{
		...
		for (const auto& x : huff.table()) {
			os.write(reinterpret_cast<const char*>(&x._sym), 1);
			os.write(reinterpret_cast<const char*>(&x._len), 1);
			os.write(reinterpret_cast<const char*>(&x._code), 4);
		}
	}

	{
		std::ofstream os("code.c");
		for (const auto& x : huff.table()) {
			os << "{ " << +x._sym << ", ";
			os << +x._len << ", ";
			os << x._code << "},\n";
	...
	std::vector<uint8_t> vec_compress = z.compress("aeio");

	std::string str_decompress = z.decompress(vec_compress);
